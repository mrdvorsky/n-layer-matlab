...
arguments
    ...
    modeStruct.WaveguideEr(1, 1) {nLayer.mustBeErUrCallable} = @(f) 1 + 0*f;
    modeStruct.WaveguideUr(1, 1) {nLayer.mustBeErUrCallable} = @(f) 1 + 0*f;
...
if isnumeric(modeStruct.WaveguideEr)
    ...
    modeStruct.WaveguideEr = @(f) er + 0*f;
end

if isnumeric(modeStruct.WaveguideUr)
    ur = modeStruct.WaveguideUr;
    modeStruct.WaveguideUr = @(f) ur + 0*f;
end
