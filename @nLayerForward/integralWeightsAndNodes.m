...
%   Usage:
%       [q, nodes, weights] = integralVectorized(fun, a, b);
%
...
arguments
    ...
    options.MaxIntervalCount {mustBeNumeric} = 10000;
    options.MaxFunctionEvaluations {mustBeNumeric} = 10000;
    options.TolFun = @defaultTolFun;
end
...
while true
    ...
    AbsTolTmp = options.TolFun(q, options.RelTol, options.AbsTol);
...
function [AbsTolOut] = defaultTolFun(q, RelTol, AbsTol)
    AbsTolOut = max(RelTol .* abs(q), AbsTol);
end
