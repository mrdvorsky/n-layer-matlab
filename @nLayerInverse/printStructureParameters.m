...
if formatOptions.ShowLimits
    ...
    for ii = 1:length(thk)
        ...
        options.AdditionalText(ii, 4, 1) = sprintf(" [%s - j%s,", ...
            sprintf(options.FormatString(ii, 6), O.urRange(1, ii)), ...
            sprintf(options.FormatString(ii, 7), O.urpRange(1, ii)));
        
        options.AdditionalText(ii, 5, 1) = sprintf(" [%s - j%s,", ...
            sprintf(options.FormatString(ii, 8), O.urRange(1, ii)), ...
            sprintf(options.FormatString(ii, 9), O.urpRange(1, ii)));
        ...
        options.AdditionalText(ii, 4, 2) = sprintf("  %s - j%s]", ...
            sprintf(options.FormatString(ii, 6), O.urRange(2, ii)), ...
            sprintf(options.FormatString(ii, 7), O.urpRange(2, ii)));
        
        options.AdditionalText(ii, 5, 2) = sprintf("  %s - j%s]", ...
            sprintf(options.FormatString(ii, 8), O.urRange(2, ii)), ...
            sprintf(options.FormatString(ii, 9), O.urpRange(2, ii)));
    end
end
...
options.FormatString(O.urLayersToSolve, 4) = compose(...
    formatOptions.SolveParameterFormatString, ...
    options.FormatString(O.urLayersToSolve, 5));

options.FormatString(O.urpLayersToSolve, 6) = compose(...
    formatOptions.SolveParameterFormatString, ...
    options.FormatString(O.urpLayersToSolve, 7));
